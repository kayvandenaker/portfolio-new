...
export default function GridItem({ src, title, info, link, poster }) {
  ...
  return (
    <Link className={"gridItem"} href={"/" + link} key={"/" + link}>
      <div className="gridItemMedia">{src.endsWith("mp4") && !isTabletOrMobile ? <video width="100%" loop playsInline muted src={src} type="video/mp4" poster={poster} onMouseOver={(event) => event.target.play()} onMouseOut={(event) => event.target.pause()} /> : <img src={poster ? poster : src} />}</div>
